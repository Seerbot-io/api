...
@router.post("/chat", tags=group_tags, status_code=status.HTTP_204_NO_CONTENT)
def save_chat(
    request: schemas.SaveChatRequest, db: Session = Depends(get_db)
) -> Response:
    """
    ...
    try:
        ...
        user = (
            db.query(User).filter(User.wallet_address == request.wallet_address).first()
        )
