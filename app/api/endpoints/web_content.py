...
from app.schemas.web_content import Partner, Statistics

router = APIRouter()
group_tags: List[str | Enum] = ["content"]
...
def get_statistics(db: Session = Depends(get_db)) -> Statistics:
    ...
    join (select sum(usd_value) liquidity, count(*) n_tx
    ...
    if result is None:
        return Statistics(n_pair="0", liquidity="0", n_tx="0")
